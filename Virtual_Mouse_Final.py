...
while True:
    ...
    if len(lmList) != 0:
        ...
        if fingers[0]==0 and fingers[1]==0 and fingers[2]==0 and fingers[3]==0 and fingers[4]==0:
            # distThumbIndex, img, lineInfo = detector.findDistance(4, 8, img)
            # distIndexMiddle, img, lineInfo = detector.findDistance(8, 12, img)
            # distMiddleRing, img, lineInfo = detector.findDistance(12, 16, img)
            # distRingLittle, img, lineInfo = detector.findDistance(16, 20, img)
            # print(distThumbIndex, distIndexMiddle, distMiddleRing, distRingLittle)
            # 121.8400590938793 39.66106403010388 34.785054261852174 53.075418038862395
            # if(distIndexMiddle <= 39 and distMiddleRing <= 34 and distRingLittle <= 53):
            #     cv2.circle(
            #         img, (lineInfo[4], lineInfo[5]), 15, (255, 0, 255), cv2.FILLED
            #     )
            #     autopy.key.tap(autopy.key.Code.SPACE)
